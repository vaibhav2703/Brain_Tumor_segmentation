...
def train(model, data_in, loss, optim, max_epochs, model_dir, test_interval=1 , device=torch.device("cpu")):
    ...
    for epoch in range(max_epochs):
        ...
        for batch_data in train_loader:
            ...
            volume, label = batch_data["image"].to(device), batch_data["label"].to(device)
            ...
            print(f"Input tensor shape: {volume.shape}")
        ...
        if (epoch + 1) % test_interval == 0:
            ...
            with torch.no_grad():
                ...
                for test_data in test_loader:
                    ...
                    test_volume = test_volume.unsqueeze(1)
...
def trainthemodel(model, data_in, loss, optim, max_epochs, model_dir, test_interval=1 , device=torch.device("cpu")):
    best_metric = -1
    best_metric_epoch = -1
    save_loss_train = []
    save_loss_test = []
        ...
        if (epoch + 1) % test_interval == 0:
            ...
            with torch.no_grad():
                ...
                for test_data in test_loader:
                    ...
                    sw_batch_size = 4
                    test_outputs = sliding_window_inference(test_volume, roi_size,sw_batch_size, model)
                    # test_outputs = [post_trans(i) for i in deco]
                    dice_metric(test_outputs, test_label)
